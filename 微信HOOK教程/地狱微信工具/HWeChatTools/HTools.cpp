...
vector<string> HTools::Split(const string& str, const string& delim) {
	vector<string> res;
	if ("" == str) return res;
...
bool HTools::StartWith(const wchar_t* str, const wchar_t* start) {
	int l1 = wcslen(str);
	int l2 = wcslen(start);
	if (l1 >= l2) {
		return wcsncmp(str, start, l2) == 0;
	}
	return false;
}

bool HTools::StartWith(const char* str, const char* start) {
	int l1 = strlen(str);
	int l2 = strlen(start);
	if (l1 >= l2) {
		return strncmp(str, start, l2) == 0;
	}
	return false;
}

bool HTools::EndWith(const wchar_t* str, const wchar_t* end) {
	bool result = false;
...
bool HTools::EndWith(const char* str, const char* end) {
	bool result = false;
	if (str != NULL && end != NULL) {
		int l1 = strlen(str);
		int l2 = strlen(end);
